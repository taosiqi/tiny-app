...
export default function TinyPNG() {
  ...
  const startCompress = () => {
    const keys = apiKeys
      ...
      .map((k) => k.trim())
      .filter(Boolean)
    if (keys.length === 0) return alert('请先填写 TinyPNG API Key')
    ...
    window.api.compressImage({ paths, apiKeys: keys })
  }
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-5">
          <label className="block text-sm font-medium text-gray-700 mb-2">
            ...
            <span className="ml-2 text-xs font-normal text-gray-400">
              ...
              <a
                ...
                onClick={(e) => {
                  e.preventDefault()
                  window.electron.shell.openExternal('https://tinify.com/developers')
                }}
              >
                tinify.com
              </a>{' '}
          ...
          <textarea
            rows={2}
            placeholder="your-api-key-1, your-api-key-2"
            value={apiKeys}
            onChange={(e) => setApiKeys(e.target.value)}
            className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 font-mono outline-none focus:border-blue-400 resize-none"
          />
        </section>
